...
compose_map={'version': '2'}
instances_map={}
>>>>>>> a1a9d485d85b0e613a624fc6feeacae5d5d3eb81


def compose_file_handle():
    for i in range(instances_num):
<<<<<<< HEAD
        instances_map["mcs" + str(i)] = {'image': image_url + ':' + image_ver,
                                         'volumes': [logpath + '/' + "mcs" + str(i) + ':' + launcher_logpath],
                                         'environment': ['SETTING_URL=' + setting_url, 'APPNAME=' + 'mcs' + str(i),
                                                         'RUNENV=' + runenv,
                                                         'IP=' + local_ip, 'KEEP_PORT=' + str(mcs_ports['KEEP_PORT']),
                                                         'HTTP_PORT=' + str(mcs_ports['HTTP_PORT'])],
                                         'ports': [
                                             str(mcs_ext_ports['KEEP_PORT'] + i) + ':' + str(mcs_ports['KEEP_PORT']),
                                             str(mcs_ext_ports['HTTP_PORT'] + i) + ':' + str(mcs_ports['HTTP_PORT'])]}
