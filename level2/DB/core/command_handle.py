    ...
    if len(insert_value_list) != len(columns_list):
        ...
        for i in columns_list:
            ...
            if dbfile_handle.columns_handle(columns, i)[0] == 'int' and insert_value_list[columns_list.index(i)] != '':
                i_val = int(insert_value_list[columns_list.index(i)].rstrip().lstrip())
            elif dbfile_handle.columns_handle(columns, i)[0] == 'float' and insert_value_list[columns_list.index(i)] != '':
                i_val = float(insert_value_list[columns_list.index(i)])
            elif dbfile_handle.columns_handle(columns, i)[0] in ('int','float') and insert_value_list[columns_list.index(i)] == '':
                i_val = None
            else:
                i_val = str(insert_value_list[columns_list.index(i)].rstrip().lstrip())
